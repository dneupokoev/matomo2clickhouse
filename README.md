...
- Ошибка OperationalError(1236, "bogus data in log event; the first event 'mysql-bin.009444' at 5559481, the last event read from 'mysql-bin.009444' at 40259915, the last byte read from 'mysql-bin.009444' at 40259934."
  ...
  - сначала сморим что случилось, для этого выполняем на mysql запрос:
  ```
  SHOW BINARY LOGS
  ...
  И видим, что нам mysql-bin.009444 явно отличается от других.
  ```
  - далее прогоняем построчно скрипт, чтобы максимально обработать строки из бинлога 'mysql-bin.009444':
  ```
  Играем значением для параметров в settings (уменьшаем и запускаем скрипт, пока не приблизимся к 40259934)
  ...
  ```
  INSERT INTO matomo.log_replication (dateid, created_at, sql_type, log_time, log_file, log_pos_start, log_pos_end) VALUES (17097906301653379, '2024-03-07 05:50:30', 'INS-UPD', '2024-03-06 20:43:16', 'mysql-bin.009445', 0, 0);
  
  dateid - должен быть МАКСИМАЛЬНЫМ

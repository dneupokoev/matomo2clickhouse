...
- Базы данных (схемы) должны называться одинаково в MySQL и ClickHouse. Поскольку в скрипте ```script_create_clickhouse_table.sql``` база называется "matomo", то и базы должны называться "matomо", либо необходимо поправить название в скрипте. Возможно в следующих версиях будет реализовано разное название баз данных, но пока так.
- Перед обновлением matomo НЕОБХОДИМО (!!!) изучить что меняется. В случае, если меняется структура базы данных, то нужно проработать план обновления (перед обновой сначала провести полный обмен с остановкой базы данных matomo, после этого привести стуртуру баз данных в соотвествите и т.д.) 
